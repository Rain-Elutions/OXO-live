...
def read_input_folder(input_dir, ext, *args, move_failed_files=True, **kwargs):
    """
    ...
    move_failed_files: bool, default = True
        whether to move failed files to the error folder 
    ...
    print(f"Moved failed files: {move_failed_files}")
...
def put_data(df, output_dir, name_to_id, sep='___', output_property_id=None):
    ...
    folderdate = '-'.join([timestamp.strftime('%Y'), timestamp.strftime('%m'), timestamp.strftime('%d')])
