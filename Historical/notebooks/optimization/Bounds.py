...
class Bounds:
    ...
    def dynamic_bounds(self, ) -> pd.DataFrame:
        ...
        max_rate_change = [item['max_rate'] for item in self.controllable_info.values()]
        
        min_dyn_bounds = current_controllable_value - (current_controllable_value*max_rate_change)
        max_dyn_bounds = current_controllable_value + (current_controllable_value*max_rate_change) + 0.0001
    ...
    def final_bounds(self, ) -> List[List[Tuple[float]]]:
        ...
        for i, cont in enumerate(controllable_full_name):
            ...
            min_dyn_bounds.loc[min_dyn_bounds[cont] > max_glo_bounds[i], cont] = max_glo_bounds[i] - 0.001
            max_dyn_bounds.loc[max_dyn_bounds[cont] < min_glo_bounds[i], cont] = min_glo_bounds[i] + 0.001
